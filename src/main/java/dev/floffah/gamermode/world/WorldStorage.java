...
public class WorldStorage {
    ...
    public void initialiseStorage() throws IOException {
        ...
        if (this.uidFile.exists()) this.readUID();
        else {
            this.world.uuid = UUID.randomUUID();
            this.writeUID();
        }
        this.getWorld()
            .getWorldManager()
            .getServer()
            .getLogger()
            .debug("UUID of " + this.world.name + " is " + this.world.uuid);
    ...
    public void saveAll() throws IOException {
        this.getWorld().getWorldManager().getServer().getLogger().info("Saving " + this.world.name);
    ...
    public UUID readUID() throws IOException {
        ...
        return this.world.uuid = UUIDUtil.intArrayToUUID(uuidParts);
    }

    public void writeUID() throws IOException {
        this.world.uuid = UUID.randomUUID();
        int[] uuidParts = UUIDUtil.uuidToIntArray(this.world.uuid);
    ...
    public void readLevel() throws IOException {
        ...
        if (!(levelNBTTag instanceof CompoundTag)) throw new IOException("Invalid level.dat");

        CompoundTag levelTag = (CompoundTag) levelNBTTag;
        CompoundTag dataTag = levelTag.getCompoundTag("Data");

        if (dataTag.containsKey("LevelName")) this.world.levelName = dataTag.getString("LevelName");
        if (dataTag.containsKey("ServerBrands")) {
            ListTag<StringTag> serverBrands = dataTag.getListTag("ServerBrands").asStringTagList();
            this.world.miscData.serverBrands.clear();
            serverBrands.forEach(brand -> {
                this.world.miscData.serverBrands.add(brand.getValue());
            });
            if (!this.world.miscData.serverBrands.contains("GamerMode"))
                this.world.miscData.serverBrands.add("GamerMode");
        }

        if (dataTag.containsKey("WorldGenSettings")) {
            CompoundTag worldGenTag = dataTag.getCompoundTag("WorldGenSettings");

            if (worldGenTag.containsKey("seed")) this.world.seed = worldGenTag.getLong("seed");
        }
    }
    ...
    public void writeLevel() throws IOException {
        ...
        this.world.miscData.serverBrands.forEach(brand -> serverBrands.add(new StringTag(brand)));
